...
流程（logscope-dev 编排）：log-triage 长日志→有界 digest+预览指针（harmony 走 hilog/HiSysEvent/faultlog 专用 parser，原始日志落 /tmp/logscope/，不灌上下文）→ wiki-reader 取调用链/契约 → code-tracer CRG 新鲜度门+search/query callers_of/impact/flow 定位 file:line → 自校正复核证据链（≤3 次）→ 报告（哪行+置信度+证据链 log行号+file:line+图边）→ 🛑人审。
