...
1. **log-triage**：长日志 → 有界 digest（错误信息 + 栈帧 + 时间线 + 关键符号，去重截断 ≤500 行）。原始日志写 `/tmp/logscope/<hash>.txt`，拿回**预览指针**（路径 + 摘要 + 关键行号）。digest 里标出 **claimed error**（日志声称的错）。
2. **wiki-reader**：读目标仓 wiki（若有）+ 源码头注释 → 调用链/契约上下文。无 wiki 则退回源码。
...
- **日志不进上下文**：只读 log-triage 的 digest/预览；要取证让 log-triage `sed -n` 回读行段，或调 code-tracer 读源码。
